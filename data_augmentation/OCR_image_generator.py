...
def get_bestcolor(color_lib, crop_lab):
    if crop_lab.size > 4800:
        crop_lab = cv2.resize(crop_lab,(200,32))  #将图像转成100*16大小的图片
    ...
    if rd < 0.3: 
        ...
        while True:              
            ...
            if f_w < w:
                ...
                # print('kk',kk)
                if (np.linalg.norm(np.reshape(np.asarray(crop_lab),(-1,3)).std(axis=0))>55 or all_in_fonts) and retry<50:  # 颜色标准差阈值，颜色太丰富就不要了
                    retry = retry+1
                    #print('retry',retry)
                    continue
...
if __name__ == '__main__':
    ...
    parser.add_argument('--font_min_size', type=int, default=25)
    ...
    parser.add_argument('--chars_file',  type=str, default='dic6278_gen.txt',
    ...
    parser.add_argument('--prydown', action='store_true', default=True,
    ...
    parser.add_argument('--output_dir', type=str, default='rec_fake9', help='Images save dir')

    parser.add_argument('--horizontal_ratio', type=float, default=0.5)
    ...
    if os.path.exists(labels_path):  # 支持中断程序后，在生成的图片基础上继续
        ...
        #print('lines',lines[1])
        f.close()
        print(lines[-1].strip().split('.')[0].split('_'))
        gs = int(lines[-1].strip().split('.')[0].split('_')[2])
    ...
    for i in range(gs+1,cf.num_img):
        ...
        rnd = random.random()
        if rnd<cf.horizontal_ratio: # 设定产生水平文本的概率
            ...
            #gen_img = rotate(gen_img, 90)            
        save_img_name = 'newfimg3_' + str(i).zfill(7) + '.jpg'
        ...
        gen_img.save('./'+output_dir+save_img_name)
        f.write(output_dir+save_img_name+ '\t'+chars+'\n')
